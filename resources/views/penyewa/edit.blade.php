...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit Penyewa</h1>
        <p class="text-gray-600 mt-2">Ubah data penyewa {{ $penyewa->nama_lengkap }}</p>
    </div>

    <form action="{{ route('penyewa.update', $penyewa->id) }}" method="POST" class="bg-white shadow-md rounded-lg p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $penyewa->nama_lengkap) }}" 
        ...
        <div>
            ...
            <input type="text" id="nomor_telepon" name="nomor_telepon" value="{{ old('nomor_telepon', $penyewa->nomor_telepon) }}" 
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
            @error('nomor_telepon')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" id="nomor_ktp" name="nomor_ktp" value="{{ old('nomor_ktp', $penyewa->nomor_ktp) }}" 
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
            @error('nomor_ktp')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea id="alamat_asal" name="alamat_asal" rows="4" 
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>{{ old('alamat_asal', $penyewa->alamat_asal) }}</textarea>
            @error('alamat_asal')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
